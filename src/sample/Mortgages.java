...
public class Mortgages {
    ...
    public static void mortgages() {
        Label lblMortgage = new Label("Mortgage Amount :");
        lblMortgage.setLayoutX(40);
        lblMortgage.setLayoutY(97);
        lblMortgage.setMinWidth(97);
        lblMortgage.setMinHeight(17);
        lblMortgage.setFont(Font.font("SansSerif", FontWeight.BOLD, 14));
        lblMortgage.setStyle("-fx-text-fill: #1cc9c7;");
        TextField tfMortgage = new TextField();
        tfMortgage.setLayoutX(190);
        tfMortgage.setLayoutY(94);
        tfMortgage.setMaxWidth(97);
        tfMortgage.setMinHeight(25);
        tfMortgage.setStyle("-fx-background-radius: 20;");
        ...
        Label lblNum = new Label("Mortgage Term :");
        lblNum.setLayoutX(58);
        ...
        File file = new File("MortgageRecords.txt");
        ...
        calculate.setOnAction(event -> {
            try {
                ...
                if (tfMortgage.getText().isEmpty()) {
                    String num = tfNum.getText();
                    String payment = tfPayment.getText();
                    double P = (Double.parseDouble(payment) / (interestRate / compounds)) * (1 - (1 / Math.pow(1 + (interestRate / compounds), Double.parseDouble(num) * 12)));
                    tfMortgage.setText("" + Math.round(P * 100.0) / 100.0);
                } else if (tfNum.getText().isEmpty()) {
                    String mortgage = tfMortgage.getText();
                    String payment = tfPayment.getText();
                    double t = Math.log((Double.parseDouble(payment) / (interestRate / compounds)) / (Double.parseDouble(payment) / (interestRate / compounds) - Double.parseDouble(mortgage))) / Math.log(1 + (interestRate / compounds));
                    tfNum.setText("" + Math.round(t / 12));
                } else if (tfPayment.getText().isEmpty()) {
                    String mortgage = tfMortgage.getText();
                    String num = tfNum.getText();
                    double PMT = (Double.parseDouble(mortgage) * (interestRate / compounds) * Math.pow(1 + (interestRate / compounds), Double.parseDouble(num) * 12)) / (Math.pow(1 + (interestRate / compounds), Double.parseDouble(num) * 12) - 1);
            ...
            try {
                ...
                printWriter.println(tfMortgage.getText());
        ...
        retrieve.setOnAction(event -> {
            TextField[] textFields = {tfMortgage, tfInterest, tfNum, tfPayment};
        ...
        keyboard.installListener(tfMortgage, tfInterest, tfNum, tfPayment);
        ...
        root.getChildren().add(lblMortgage);
        root.getChildren().add(tfMortgage);
